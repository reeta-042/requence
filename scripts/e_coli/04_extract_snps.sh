#!/bin/bash
# =============================================================
# E. COLI — SCRIPT 04: SNP EXTRACTION (Snippy)
# Input:  query_genome.fna
#         $REFERENCE  (escherichia_coli.gbff — injected by API)
# Output: snippy_production_out/snps.tab
#         logs/04_extract_snps.log
# NOTE:   Reference defaults to /app/reference/escherichia_coli.gbff
#         This is the slowest step (~10-20 min for a full assembly).
# =============================================================

set -euo pipefail

WORK_DIR="${WORK_DIR:-$(pwd)}"
REFERENCE="${REFERENCE:-/app/reference/escherichia_coli.gbff}"
...
{
echo "============================================================"
echo "E. COLI SCRIPT 04: SNP EXTRACTION (Snippy)"
...
if [ ! -f "$REFERENCE" ]; then
    ...
    echo "  Set the REFERENCE env variable or place escherichia_coli.gbff" | tee -a "$LOG_FILE"
    echo "  in /app/reference/" | tee -a "$LOG_FILE"
    exit 1
fi
...
# ── Remove stale Snippy output (ensures idempotency) ─────────
...
if [ ! -f "$SNIPPY_OUTDIR/snps.tab" ]; then
    echo "  ⚠ snps.tab not produced — genome may have no detectable variants vs reference" | tee -a "$LOG_FILE"
    ...
    echo "  ⚠ Created empty snps.tab — SNP features will all be 0" | tee -a "$LOG_FILE"
else
