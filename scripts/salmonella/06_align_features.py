#!/usr/bin/env python3
# =============================================================
# SALMONELLA — SCRIPT 06: FEATURE ALIGNMENT
...
with open(LOG_FILE, "w") as fh:
    fh.write("=" * 60 + "\n")
    fh.write("SALMONELLA SCRIPT 06: FEATURE ALIGNMENT\n")
    fh.write(f"Started: {datetime.now()}\n")
    fh.write("=" * 60 + "\n")

# ── 1. Load extracted feature files ────────────────────────────
log("Loading extracted feature files...")

required = [
    "gene_presence_production.csv",
    "kmer_production.csv",
    "snp_production.csv",
]
...
log(f"  Gene features extracted : {genes_df.shape[1] - 1:,}")
log(f"  K-mer features extracted: {kmers_df.shape[1] - 1:,}")
log(f"  SNP features extracted  : {snps_df.shape[1] - 1:,}")

# ── 2. Merge into one raw feature row ──────────────────────────
log("\nMerging all extracted features...")
...
# ── 4. Run alignment ────────────────────────────────────────────
# Salmonella has ONE shared feature template for all 8 antibiotics.
# All models consume aligned_full.csv.
log("\n" + "=" * 60)
