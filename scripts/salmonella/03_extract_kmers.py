#!/usr/bin/env python3
# =============================================================
# SALMONELLA — SCRIPT 03: K-MER EXTRACTION (tBLASTn)
# Input:  blast_db/query_genome  (from script 02)
#         $CARD_PROTEIN_FILE      (shared CARD protein FASTA)
#         $TEMPLATES_DIR/features_*.txt  (Salmonella templates)
# Output: kmer_production.csv
#         logs/03_extract_kmers.log
# NOTE:   Salmonella uses ONE shared feature set.  We scan all
...
with open(LOG_FILE, "w") as fh:
    fh.write("=" * 60 + "\n")
    fh.write("SALMONELLA SCRIPT 03: K-MER EXTRACTION\n")
...
# ── 1. Collect training k-mers from ALL Salmonella templates ──
...
if len(training_kmers) == 0:
    log("✗ ERROR: No k-mers found in templates. Check TEMPLATES_DIR.")
    sys.exit(1)
...
# ── 3. Filter CARD proteins to detected resistance genes ───────
...
if len(filtered_proteins) == 0:
    log("✗ ERROR: No proteins matched resistance genes. Check CARD file and gene list.")
    sys.exit(1)

# ── 4. Run tBLASTn ─────────────────────────────────────────────
log("\nRunning tBLASTn (this may take several minutes)...")
...
# ── 7. Extract k-mers from BLAST subject sequences ────────────
...
# Filter to training k-mers only
matched_kmers = {k: v for k, v in found_kmers.items() if k in training_kmers}
log(f"  K-mers matching training : {len(matched_kmers):,} / {len(training_kmers):,}")
missing_pct = (1 - len(matched_kmers) / max(len(training_kmers), 1)) * 100
log(f"  Missing k-mers           : {missing_pct:.1f}%  (filled with 0 in alignment step)")
